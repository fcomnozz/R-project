...
contribution, by calling MAGELLAN in `fl_statistics` .
...
You can also use the Ising model [eg., @ferretti_measuring_2016; @brouillet_magellan:_2015], 
all loci are arranged sequentially, and each locus interacts with its physical neighbors.
The last and the first loci will interact only if 'circular' is set. For each pair of 
interacting loci, there is an associated cost if both alleles are not identical 
(and therefore 'compatible'). 
We call the fuction `fl_generate` from MAGELLAN [@brouillet_magellan:_2015].

```{r ISex1}
ris <- rfitness(5, I = 2, i = 1, model = "Ising")
plot(ris)
...
With the Full model [e.g., @ferretti_measuring_2016; @brouillet_magellan:_2015], 
the fitness is computed with different parts of other models depending on the 
choosen parameters. We call the fuction `fl_generate` from MAGELLAN [@brouillet_magellan:_2015].
